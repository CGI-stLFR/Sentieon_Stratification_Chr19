...
def get_all_output_files():
  ...
  for sample in config["samples"]:
    ...
    for ref_name in ("hs37d5", ):
      ...
      for truth_hap in ['hapA', 'hapB']:
        expected_files.append("switch_error_logs/" + sample + '/' + truth_hap + '/' + ref_name + "/switch_results.txt")
...
rule bin_synthetic_align:
  input:
    ...
    truth_fa = config["truth_dir"] + "chr19_normalized_{truth_hap}.fa",
...
rule generate_var_files:
    ...
    shell:
        """
        paftools.js call <(zcat {input.aligned} | sort -k6,6 -k8,8n) > {output.varfile}
        """
...
rule calculate_switch_errs:
    ...
    shell:
        "multi_vcf_switch_errors.py {input.varfile} {input.vcf} $(dirname {output.switches}) > {output.switches}; "
        "grep ^R {input.varfile} | awk '{{sum+=$4-$3}} END {{print (sum/59128983)*100}}' >> {output.switches}"

################################
# Actually perform the binning #
################################

rule bin_without_pid:
  input:
    ...
    bin_without_pid_script = config["bin_without_pid_script"]
  output:
  ...
  shell:
    ...
    mkdir -p $outdir
    python3 {input.bin_without_pid_script} {input.input_fa} {input.aligned} {output.binned_results}
    """
